				...
				while(data_k.length > 0) {
					...
					div.selectAll('path')
						...
						.on('click',function(d,i,n){
							/*pass information to text pane*/
						})
						.on('mousemove',function(d,i,n){
							var ttp =  '<p><b>SUPERKLASS</b> <span>'+ d.superklass +'</span></p>'
								ttp += '<p><b>KLASS</b> <span>' + d.klass + '</span></p>'
								ttp += '<p><b>SUBKLASS</b> <span>' + d.subklass + '</span></p>'
							d3.select('#tooltip')
								.style('display','block')
								.style('left', d3.event.pageX+5+'px')
								.style('top', d3.event.pageY-16+'px')
							d3.select('#tooltip').select(".title").text(d.subklass);
